...
const IMAGE_DIR = path.join("data", "meko-catalogue", "product-images");
...
const DEFAULT_CATEGORY = "Water Pump Assemblies";
const PROTECTED_BRANDS = ["pensol", "menon", "menon brakes", "oah", "ci automotive llp"];
const PART_NUMBER_RE = /^M-\d{2,5}(?:\s+[A-Z0-9]{1,4}){0,3}$/i;
...
function splitPartNumbers(raw: string): string[] {
  return String(raw || "")
    .split("/")
...
function findSourceImage(product: ExtractedProduct): string | null {
  const keys = [product.imageKey, product.imageFile, slugify(product.cataloguePartNumber).replace(/-/g, "_")].filter(
    Boolean,
  ) as string[];
  for (const key of keys) {
    const base = path.basename(String(key), path.extname(String(key)));
...
function copyProductImage(partNumber: string, source: string | null) {
  if (!source || !existsSync(source)) return null;
  const ext = path.extname(source).toLowerCase() || ".jpg";
  const destName = `${partNumber.replace(/[^A-Za-z0-9._-]+/g, "_")}${ext}`;
  const dest = path.join(STORE_DIR, destName);
...
async function main() {
  ...
  for (const row of extracted.products || []) {
    const numbers = splitPartNumbers(row.cataloguePartNumber);
    if (!numbers.length) {
      products.push({ ...row, cataloguePartNumber: "", validationStatus: "REVIEW" });
      continue;
    }
    for (const cataloguePartNumber of numbers) {
      products.push({
        ...row,
        cataloguePartNumber,
        reviewReasons: [
          ...(row.reviewReasons || []),
          ...(numbers.length > 1 ? ["split_from_combined_catalogue_card"] : []),
        ],
      });
    }
  }
  ...
  for (const product of products) {
    ...
    if (!PART_NUMBER_RE.test(pn) || application.length < 4) {
  ...
  const categoryCache = new Map(
    existingCategories.map((row) => [row.name.toLowerCase(), row] as const),
  );
  async function ensureCategory(name: string) {
    const label = String(name || DEFAULT_CATEGORY).trim() || DEFAULT_CATEGORY;
    ...
    await db.insert(partCategory).values({
      ...
      name: label,
      slug: finalSlug,
      description: `${label} from the MEKO Genuine Spares photo catalogue.`,
    });
  ...
  for (const group of chunk(ready, 20)) {
    ...
    for (const row of group) {
      ...
      const category = await ensureCategory(row.category || DEFAULT_CATEGORY);
      const imageName = copyProductImage(row.cataloguePartNumber, findSourceImage(row));
  ...
  if (!typesenseDocs.length) {
    ...
    for (const row of existingMeko) {
      typesenseDocs.push({
        ...
        category: (row.categoryId && catName.get(row.categoryId)) || DEFAULT_CATEGORY,
        vehicle_ids: [],
      });
    }
  }
